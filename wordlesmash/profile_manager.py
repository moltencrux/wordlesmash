...
class Profile:
    ...
    # rebuild_trees # if words content chanages : just clear the treees.. or put them back to inintial picks
    # game type changed -> DT

    # what kinds of actions invalidate trees?
        # adding/removing picks
        # adding/removing candidates
        #

    # what changes don't invalidate trees?
        # renaming profile w/o trees, but trees must be rewrititen

    # needs rewriting QSettings
        # adding/removing initial_picks
    
    # can trees be dirty/wo words changing? yes, (game type change only)

    # if we invalidate all trees, then i think we should rewrite words?

    # words need saving (on rename), but doesn't invalidate tree. we clear dict in that case

    # words change:
        # clear dt (move to IP)
        # words modified

    #** settings change (rename) (full save)
        # words modified (but not dt)
        # settings modified

    # settings change (game type)
        # clear dt (move to IP)
        # words modified (but not dt)
